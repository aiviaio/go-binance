...
// Endpoints - Updated to new URL structure (deadline 2026-04-23)
...
const (
	// Legacy URLs (deprecated, will be removed after 2026-04-23)
...
// Deprecated: Use getWsMarketEndpoint or getWsPrivateEndpoint instead
...
// getWsMarketEndpoint returns the market data WebSocket endpoint
...
// Deprecated: !ticker@arr will be retired on 2026-03-26. Use WsAllMiniMarketTickerServe (!miniTicker@arr) or WsMarketTickerServe (<symbol>@ticker) instead.
...
func WsBookTickerServe(symbol string, handler WsBookTickerHandler, errHandler ErrHandler) (doneC, stopC chan struct{}, err error) {
	endpoint := fmt.Sprintf("%s/%s@bookTicker", getWsMarketEndpoint(), strings.ToLower(symbol))
	cfg := newWsConfig(endpoint)
	wsHandler := func(message []byte) {
...
func WsAllBookTickerServe(handler WsBookTickerHandler, errHandler ErrHandler) (doneC, stopC chan struct{}, err error) {
	endpoint := fmt.Sprintf("%s/!bookTicker", getWsMarketEndpoint())
	cfg := newWsConfig(endpoint)
	wsHandler := func(message []byte) {
		event := new(WsBookTickerEvent)
		err := json.Unmarshal(message, &event)
...
func wsDepthServe(symbol string, levels string, rate *time.Duration, handler WsDepthHandler, errHandler ErrHandler) (doneC, stopC chan struct{}, err error) {
	...
	endpoint := fmt.Sprintf("%s/%s@depth%s%s", getWsMarketEndpoint(), strings.ToLower(symbol), levels, rateStr)
	cfg := newWsConfig(endpoint)
...
func WsAvgPriceServe(symbol string, handler WsAvgPriceHandler, errHandler ErrHandler) (doneC, stopC chan struct{}, err error) {
	endpoint := fmt.Sprintf("%s/%s@avgPrice", getWsMarketEndpoint(), strings.ToLower(symbol))
	cfg := newWsConfig(endpoint)
	wsHandler := func(message []byte) {
